...
from pytoniq_core.tlb.custom.wallet import HighloadWalletV3Data, WalletMessage  # # pyright: ignore [reportMissingTypeStubs]
...
class HighloadWalletV3(BaseWallet):
    ...
    @staticmethod
    def create_data_cell(public_key: bytes, wallet_id: int, timeout: int,
                         old_queries: dict[int, Cell] | None = None, queries: dict[int, Cell] | None = None) -> Cell:
    ...
    @classmethod
    def from_params(cls, public_key: bytes, wc: int = 0, wallet_id: int | None = None, timeout: int | None = None, private_key: bytes | None = None) -> Self:
        ...
        return cls(address=address, state_init=state_init, private_key=private_key, wallet_id=wallet_id, timeout=timeout)

    @classmethod
    def from_private_key(cls, private_key: bytes, wc: int = 0, wallet_id: int | None = None, timeout: int | None = None):
        public_key: bytes = private_key_to_public_key(private_key)
        return cls.from_params(public_key=public_key, wc=wc, wallet_id=wallet_id, timeout=timeout, private_key=private_key)
    ...
    def raw_create_transfer_msg(self, private_key: bytes, wallet_id: int, messages: list[WalletMessage],
                                query_id: int | None = None, timeout: int | None = None, created_at: int | None = None) -> Cell:
    ...
    async def transfer(self, provider: Provider, msgs: list[WalletMessage], query_id: int | None = None, timeout: int | None = None):
    ...
    async def send_init_external(self, provider: Provider):
        ...
        assert self.private_key is not None, 'must specify wallet private key!'
        body = self.raw_create_transfer_msg(private_key=self.private_key, wallet_id=self.wallet_id, messages=[self.create_wallet_internal_message(destination=self.address, send_mode=3, value=0)])
