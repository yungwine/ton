...
def _stats_logs() -> list[Path]:
    ...
    return sorted((root / "test" / "integration" / "stats_dir").glob("session-logs*"))
...
def test_figure_builder_includes_crosslink_markers():
    ...
    summary = builder.build_summary("0,8000000000000000.1", 0, 50, False)
    detail = builder.build_detail("0,8000000000000000.1", 10, "abs")

    summary_names = {trace.name for trace in summary.data}
    detail_names = {trace.name for trace in detail.data}

    assert "mc_ref_collate_started" in summary_names
    assert "mc_ref_finalized" in summary_names


def test_cached_parser_keeps_crosslink_markers_in_stats_dir_mode():
    stats_dir = Path(__file__).resolve().parents[4] / "test" / "integration" / "stats_dir"
    db_path = stats_dir / f"test-index-{uuid4().hex}.db"
    try:
        ...
        data = parser.parse_group("0,8000000000000000.1")
