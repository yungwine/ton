...
from consensus_explorer.models import ConsensusData, GroupData, GroupInfo, UnnamedGroupInfo
...
class RecordingTonlibClient:
    ...
    async def get_block_transactions(self, block) -> list[int]:
        if self.fail_transactions:
            raise RuntimeError("boom")
        return [1, 2]

    async def get_block_header(self, block) -> FakeHeader:
        return FakeHeader()
...
def test_parser_session_stats_parse_group_reuses_filtered_result_for_same_snapshot(monkeypatch):
    group = UnnamedGroupInfo(valgroup_hash=b"a", group_start_est=1.0)
...
def test_format_slot_meta_uses_fresh_tonlib_client_for_each_lookup():
    factory = RecordingTonlibFactory()
    app = DashApp(DummyParser([]))
    app._tonlib_factory = factory
