...
from consensus_explorer.models import UnnamedGroupInfo
...
def test_remove_file_deletes_groups_without_remaining_files(tmp_path, monkeypatch):
    ...
    groups_by_file: dict[Path, set[UnnamedGroupInfo]] = {
    ...
    monkeypatch.setattr(
        index, "_scan_file", lambda path: FileScanResult(groups=groups_by_file[path.resolve()])
    )
...
def test_index_file_removes_deleted_file_if_it_disappears_before_reindex(tmp_path, monkeypatch):
    stats_dir = tmp_path / "stats"
    stats_dir.mkdir()
    db_path = tmp_path / "index.db"
    ...
    index = FileIndex(stats_dir, db_path)
    monkeypatch.setattr(
        index, "_scan_file", lambda path: FileScanResult(groups={_group(b"g-only-a")})
    )
...
def test_index_file_skips_directory_and_removes_stale_entry(tmp_path, monkeypatch):
    stats_dir = tmp_path / "stats"
    stats_dir.mkdir()
    db_path = tmp_path / "index.db"
...
def test_initial_scan_skips_directories(tmp_path, monkeypatch):
    stats_dir = tmp_path / "stats"
    stats_dir.mkdir()
    db_path = tmp_path / "index.db"
    ...
    groups_by_file: dict[Path, set[UnnamedGroupInfo]] = {file_a.resolve(): {_group(b"g-only-a")}}
...
def test_scan_file_skips_model_errors(tmp_path):
    db_path = tmp_path / "index.db"
    stats_dir = tmp_path / "stats"
    stats_dir.mkdir()
    ...
    log_file.write_text(f'{{"@type":"consensus.stats.events","id":1,"events":[]}}\n{valid_line}\n')
