...
td::Result<tonlib::FFIEngineConsoleClient> create_ffi_client(TonlibEventLoop *loop, char const *config) {
  ...
  // FIXME: public_key_ and private_key_ can be null.
  auto public_key_slice = ton::serialize_tl_object(parsed_config.public_key_.get(), true);
  auto private_key_slice = ton::serialize_tl_object(parsed_config.private_key_.get(), true);

  td::IPAddress parsed_address;
  TRY_STATUS(parsed_address.init_host_port(parsed_config.address_));
  TRY_RESULT(parsed_public_key, ton::PublicKey::import(public_key_slice));
  TRY_RESULT(parsed_private_key, ton::PrivateKey::import(private_key_slice));

  return tonlib::FFIEngineConsoleClient{*loop, parsed_address, parsed_public_key, parsed_private_key};
}
