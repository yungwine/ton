...
class ClientWrapper : public EngineConsoleClient {
 public:
  ClientWrapper(td::IPAddress address, ton::PublicKey public_key, ton::PrivateKey private_key,
                td::unique_ptr<td::Guard> actor_counter)
      : EngineConsoleClient(address, public_key, private_key), actor_counter_(std::move(actor_counter)) {
  }
