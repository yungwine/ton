...
class PoolImpl : public runtime::SpawnsWith<Bus>, public runtime::ConnectsTo<Bus> {
 ...
 public:
  ...
  void handle(BusHandle, std::shared_ptr<const Start>) {
    auto &bus = *owning_bus();
    owning_bus().publish<TraceEvent>(consensus::stats::Id::create(bus.local_id.idx.value(), bus.validator_set.size(),
                                                                  bus.local_id.weight, bus.total_weight,
                                                                  bus.simplex_config.slots_per_leader_window));
